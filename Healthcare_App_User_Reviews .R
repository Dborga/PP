...
plot1 <- ggplot(data = review_counts, aes(x = Healthcare.App, y = Count)) +
...
plot2 <- ggplot(data = average_rating, aes(x = Healthcare.App, y = Avg.Rating)) +
  geom_bar(stat = "identity") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
plot3 <- ggplot(data = reviews_df, aes(x = Rating)) +
...
plot4 <- ggplot(data = reviews_per_day, aes(x = Date, y = Count)) +
  geom_line() +
